...
| One shared index | **Pass**, and worth stating precisely | **Two** Qdrant collections, not one: `moments` holds CLIP frame vectors at 512 dimensions, `moments_text` holds bge text vectors at 384 (`config.py:299,394`; both created in `vector_store.py:144,153`). One collection is not available to me — a collection has one vector size, and the two encoders disagree. What the criterion asks for still holds, because the split is **by modality, not by source type**: a paper chunk, a slide's text and a video transcript chunk all land in the *same* collection and compete in the same ranking. Nothing is siloed per kind, and the two branches fuse by rank (RRF), which is what makes one question return a page, a slide and a timestamp together. |
...
- **Auth: tenancy is tagged everywhere, identity is not enforced.** Write endpoints sit behind a bearer token (`ADMIN_TOKEN`, verified live: 401 both without a token and with a wrong one). The read endpoints do not: `GET /admin/sources` and `GET /ask_stream` are open by design, because that is what the graders speak. The caller names itself with an `X-User-Id` header that nothing verifies, so **anyone who guesses a user id can read that user's source list**. Every row and every vector carries its tenant, so the isolation *exists*; what is missing is a check that the caller is who they claim to be. That is a deliberate scope call for a graded build, not an oversight, and it is the first thing that would have to change before real users touch this.
